from alocvx.engrenagens.troca_vaga_contrato import TrocaVagaEngrenagem
from alocvx.entidades.morador import Morador
from typing import List


class TrocaVagaEngrenagemFabrica:
    @staticmethod
    def criar_troca_vaga(
        tipo_engrenagem, moradores: List[Morador]
    ) -> TrocaVagaEngrenagem:
        match tipo_engrenagem:
            case "FullScanEngrenagem":
                classe = __import__(
                    "alocvx.engrenagens.full_scan", fromlist=[tipo_engrenagem]
                )
                return classe.FullScanEngrenagem(moradores)
            case "VizinhoMaisDistanteEngrenagem":
                classe = __import__(
                    "alocvx.engrenagens.vizinho_mais_distante",
                    fromlist=[tipo_engrenagem],
                )
                return classe.VizinhoMaisDistanteEngrenagem(moradores)
            case "PermutaVagaAleatorio":
                classe = __import__(
                    "alocvx.engrenagens.permuta_vaga_aleatorio",
                    fromlist=[tipo_engrenagem],
                )
                return classe.PermutaVagaAleatorio(moradores)
            case _:
                raise ValueError(f"Tipo de engrenagem desconhecido: {tipo_engrenagem}")
